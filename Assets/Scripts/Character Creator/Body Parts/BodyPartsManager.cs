        ...
        for (int partIndex = 0; partIndex < bodyPartTypes.Length; partIndex++)
        {
            ...
            for (int stateIndex = 0; stateIndex < characterStates.Length; stateIndex++)
            {
                ...
                for (int directionIndex = 0; directionIndex < characterDirections.Length; directionIndex++)
                {
                    ...
                    string resourceStr = "PlayerAnimations/" + partType + "/" + partType + "_" + partID + "_" + state + "_" + direction;
                    animationClip = Resources.Load<AnimationClip>(resourceStr);
                    ...
                    if (gameObject.name == "Player")
                    {
                        ...
                        {
                            Debug.Log("animationClip ES NULL   " + partType + " " + partID + " " + state + " " + direction);
                        }
                        else
                        {
                            Debug.Log("BIEN: " + partType + " " + partID + " " + state + " " + direction);
                        }
                    }
                    ...
                    {
                        Debug.LogError("Error al cargar: " + resourceStr + " " + gameObject.name);
                        Debug.LogError("NULL?? " + (animationClip == null).ToString() + " " + (defaultAnimationClips == null).ToString());
                    }
